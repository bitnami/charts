...
- name: wait-for-workers
  ...
  args:
    ...
    - |
      ...
      [[ -f "/opt/bitmoa/scripts/deepspeed/entrypoint.sh" ]] && source "/opt/bitmoa/scripts/deepspeed/entrypoint.sh"
  ...
  volumeMounts:
    ...
    - name: ssh-client-private-key
      mountPath: /bitmoa/ssh/client-private-key
    - name: empty-dir
...
- name: ssh-client-configure
  ...
  args:
    ...
    - |
      ...
      [[ -f "/opt/bitmoa/scripts/deepspeed/entrypoint.sh" ]] && source "/opt/bitmoa/scripts/deepspeed/entrypoint.sh"
      cp /etc/ssh/ssh_config /bitmoa/ssh/ssh-config
      if [[ ! -d /etc/ssh/ssh_config.d ]]; then
        # Older version of ssh, add the include directive
        echo "Modifying ssh_config with include directive"
        echo "Include /etc/ssh/ssh_config.d/*.conf" >> /bitmoa/ssh/ssh-config/ssh_config
      fi
  {{- if .Values.client.containerSecurityContext.enabled }}
  securityContext: {{- include "common.compatibility.renderSecurityContext" (dict "secContext" .Values.client.containerSecurityContext "context" $) | nindent 4 }}
  {{- end }}
  volumeMounts:
    - name: empty-dir
      mountPath: /bitmoa/ssh/ssh-config
...
- name: ssh-server-configure
  ...
  args:
    ...
    - |
      ...
      ssh-keygen -y -f /bitmoa/ssh/client-private-key/id_rsa > /home/deepspeed/.ssh/authorized_keys
      ...
      ssh-keygen -A -f /bitmoa/ssh/server-private-key/
      ...
      cp /etc/ssh/sshd_config /bitmoa/ssh/sshd-config
      if [[ ! -d /etc/ssh/sshd_config.d ]]; then
        ...
        while read -r line; do
          ...
          key="${entry[0]}"
          value="${entry[1]}"
          if grep -q "${entry[0]}" /bitmoa/ssh/sshd-config/sshd_config; then
            echo "Replacing ${entry[*]} in sshd_config file"
            replace_in_file /bitmoa/ssh/sshd-config/sshd_config "^[#]*${entry[0]}.*" "${entry[*]}"
          else
            echo "Adding ${entry[*]} in sshd_config file"
            echo "${entry[*]}" >> /bitmoa/ssh/sshd-config/sshd_config
          fi
        done < /bitmoa/ssh/server-configmap/*.conf
      fi
  ...
  volumeMounts:
    ...
    - name: empty-dir
      mountPath: /bitmoa/ssh/server-private-key/etc/ssh
      subPath: app-worker-private-key-dir
    - name: ssh-server-config
      mountPath: /bitmoa/ssh/server-configmap
    - name: empty-dir
      mountPath: /bitmoa/ssh/sshd-config
