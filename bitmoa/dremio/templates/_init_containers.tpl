...
- name: generate-conf
  ...
  args:
    ...
    - |
      ...
      for file in $(find /bitmoa/dremio/secrets -type f); do
      ...
      find /bitmoa/dremio/input-dremio -type f -name dremio.conf -print0 | sort -z | xargs -0 cat > /bitmoa/dremio/rendered-conf/pre-render-dremio.conf
      render-template /bitmoa/dremio/rendered-conf/pre-render-dremio.conf > /bitmoa/dremio/rendered-conf/dremio.conf
      rm /bitmoa/dremio/rendered-conf/pre-render-dremio.conf

      # Files different from dremio.conf -> Here we only apply render-template to expand the env vars
      for file in $(find /bitmoa/dremio/input-dremio -type f -not -name dremio.conf); do
          filename="$(basename $file)"
          echo "Expanding env vars from $filename"
          render-template "$file" > /bitmoa/dremio/rendered-conf/$filename
      done
  ...
  volumeMounts:
    - name: input-dremio-conf-cm
      mountPath: /bitmoa/dremio/input-dremio/dremio-conf/configmap
    {{- if .mountDremioConfSecret }}
    - name: input-dremio-conf-secret
      mountPath: /bitmoa/dremio/input-dremio/dremio-conf/secret
    {{- end }}
    - name: input-core-site
      mountPath: /bitmoa/dremio/input-dremio/core-site
    - name: empty-dir
      mountPath: /tmp
      subPath: tmp-dir
    - name: empty-dir
      mountPath: /bitmoa/dremio/rendered-conf
      subPath: app-conf-dir
    {{- if .context.Values.usePasswordFiles }}
    {{- if or .context.Values.dremio.tls.passwordSecret .context.Values.dremio.tls.password .context.Values.dremio.tls.autoGenerated.enabled .context.Values.dremio.tls.usePemCerts }}
    - name: keystore-password
      mountPath: /bitmoa/dremio/secrets/keystore-password
    {{- end }}
    {{- if or (eq .context.Values.dremio.distStorageType "minio") (eq .context.Values.dremio.distStorageType "aws") }}
    - name: s3-credentials
      mountPath: /bitmoa/dremio/secrets/s3-credentials
    {{- end }}
    {{- end }}
...
- name: init-certs
    ...
    - |
      ...
      if [[ -f "/certs/tls.key" ]] && [[ -f "/certs/tls.crt" ]]; then
          ...
          keytool -importkeystore -srckeystore "/tmp/keystore.p12" \
              ...
              -destkeystore "/opt/bitmoa/dremio/certs/dremio.jks"
          rm "/tmp/keystore.p12"
      else
      ...
      if [[ -f "/certs/dremio.jks" ]]; then
          cp "/certs/dremio.jks" "/opt/bitmoa/dremio/certs/dremio.jks"
      else
  ...
  volumeMounts:
    - name: input-tls-certs
      mountPath: /certs
    - name: empty-dir
      mountPath: /opt/bitmoa/dremio/certs
...
- name: copy-default-conf
  ...
  args:
    ...
    - |
      set -e
      echo "Copying configuration files from /opt/bitmoa/dremio/conf to empty-dir volume"
      # First copy the default configuration files so we can fully replace the folder

      cp /opt/bitmoa/dremio/conf/* /bitmoa/dremio/conf/
  volumeMounts:
    - name: empty-dir
      mountPath: /bitmoa/dremio/conf
      subPath: app-conf-dir
{{- end -}}
...
- name: upgrade-keystore
  ...
  {{- end }}
  command:
    - /opt/bitmoa/scripts/dremio/entrypoint.sh
  args:
    - dremio-admin
    - upgrade
  env:
  ...
  volumeMounts:
    ...
    - name: empty-dir
      ...
      subPath: tmp-dir
    - name: empty-dir
      mountPath: /opt/bitmoa/dremio/tmp
      subPath: app-tmp-dir
    - name: empty-dir
      mountPath: /opt/bitmoa/dremio/run
      subPath: app-run-dir
    - name: empty-dir
      mountPath: /opt/bitmoa/dremio/log
      subPath: app-log-dir
    - name: empty-dir
      mountPath: /opt/bitmoa/dremio/conf
      subPath: app-conf-dir
