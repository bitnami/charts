...
- name: wait-for-db
  ...
  env:
    ...
    - name: MASTODON_DATABASE_PASSWORD_FILE
      value: "/opt/bitmoa/mastodon/secrets/db-password"
    {{- else }}
  ...
  volumeMounts:
    ...
    - name: mastodon-secrets
      mountPath: /opt/bitmoa/mastodon/secrets
    {{- end }}
{{- end -}}
...
- name: wait-for-redis
  ...
  env:
    ...
    - name: MASTODON_REDIS_PASSWORD_FILE
      value: {{ printf "/opt/bitmoa/mastodon/secrets/%s" (include "mastodon.redis.passwordKey" .) }}
    {{- else }}
...
- name: wait-for-elasticsearch
  ...
  env:
    ...
    - name: MASTODON_ELASTICSEARCH_PASSWORD_FILE
      value: {{ printf "/opt/bitmoa/mastodon/secrets/%s" (include "mastodon.elasticsearch.passwordKey" .) }}
    {{- else }}
