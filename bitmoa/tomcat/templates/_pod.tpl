...
{{- if and .Values.volumePermissions.enabled .Values.persistence.enabled }}
  - name: volume-permissions
    ...
    command:
      ...
      - -ec
      - |
          chown -R {{ .Values.containerSecurityContext.runAsUser }}:{{ .Values.podSecurityContext.fsGroup }} /bitmoa/tomcat
    securityContext:
      runAsUser: 0
  ...
  - name: tomcat
    ...
    env:
      ...
      - name: TOMCAT_USERNAME_FILE
        value: {{ printf "/opt/bitmoa/tomcat/secrets/%s" (include "tomcat.adminUsernameKey" .) }}
      - name: TOMCAT_PASSWORD_FILE
        value: {{ printf "/opt/bitmoa/tomcat/secrets/%s" (include "tomcat.adminPasswordKey" .) }}
      {{- else }}
    ...
    volumeMounts:
      - name: data
        mountPath: /bitmoa/tomcat
      - name: empty-dir
        mountPath: /opt/bitmoa/tomcat/temp
        subPath: app-tmp-dir
      - name: empty-dir
        mountPath: /opt/bitmoa/tomcat/conf
        subPath: app-conf-dir
      - name: empty-dir
        mountPath: /opt/bitmoa/tomcat/logs
        subPath: app-logs-dir
      - name: empty-dir
        mountPath: /opt/bitmoa/tomcat/work
      ...
      {{- if  .Values.usePasswordFiles }}
      - name: tomcat-secrets
        mountPath: /opt/bitmoa/tomcat/secrets
      {{- end }}
