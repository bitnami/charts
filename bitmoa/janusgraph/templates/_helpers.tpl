...
- name: wait-for-storage
  ...
  args:
    ...
    - |
      ...
      # HACK: Gremlin console interactive mode won't work unless its history file is writable at /opt/bitmoa/janusgraph
      ...
      cp /opt/bitmoa/janusgraph/ext/* /ext-dir

      # Load env variables
      [[ -f /opt/bitmoa/scripts/janusgraph-env.sh ]] && . /opt/bitmoa/scripts/janusgraph-env.sh

      # Copy Janusgraph configmap
      cp "/bitmoa/janusgraph/conf/janusgraph.properties" "${JANUSGRAPH_PROPERTIES}"

      # Configure libnss_wrapper
      if [[ -f /opt/bitmoa/scripts/libos.sh ]]; then
      ...
      if [[ -f "/opt/bitmoa/scripts/libjanusgraph.sh" ]]; then
  ...
  env:
    ...
    - name: JANUSGRAPH_STORAGE_PASSWORD_FILE
      value: {{ printf "/opt/bitmoa/janusgraph/secrets/%s" (include "janusgraph.storage.password.secretKey" .) }}
    {{- else }}
  ...
  volumeMounts:
    ...
    - name: janusgraph-properties
      mountPath: /bitmoa/janusgraph/conf/janusgraph.properties
    ...
    - name: storage-backend-credentials
      mountPath: /opt/bitmoa/janusgraph/secrets/
    {{- end }}
{{- end -}}
