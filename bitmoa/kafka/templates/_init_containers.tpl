...
- name: prepare-config
  ...
  args:
    ...
    - |
      ...
      [[ -f "/bitmoa/kafka/data/meta.properties" ]] && ID="$(grep "node.id" /bitmoa/kafka/data/meta.properties | awk -F '=' '{print $2}')"
      ...
      if [[ "controller" =~ "$POD_ROLE" ]]; then
          ...
          for ((i = 0; i < {{ int .context.Values.controller.replicaCount }}; i++)); do
              ...
              [[ $i -eq $POD_ID ]] && [[ -f "/bitmoa/kafka/data/meta.properties" ]] && DIR_ID="$(grep "directory.id" /bitmoa/kafka/data/meta.properties | awk -F '=' '{print $2}')"
  ...
  volumeMounts:
    ...
    - name: kafka-sasl
      mountPath: /opt/bitmoa/kafka/config/secrets
      readOnly: true
    {{- end }}
{{- end -}}
