...
{{- define "common.errors.upgrade.passwords.empty" -}}
  ...
  {{- if and $validationErrors .context.Release.IsUpgrade -}}
    ...
    {{- $errorString = print $errorString "\n                 Further information can be obtained at https://docs.bitmoa.com/general/how-to/troubleshoot-helm-chart-issues/#credential-errors-while-upgrading-chart-releases" -}}
    {{- $errorString = print $errorString "\n%s" -}}
...
{{- range .images -}}
  ...
  {{- if not (contains $fullImageNameNoTag $originalImages) -}}
    ...
    {{- else if not (contains .repository $originalImages) -}}
      ...
      {{- if contains "docker.io/bitmoalegacy/" $fullImageNameNoTag -}}
        {{- $bitmoaLegacyImages = append $bitmoaLegacyImages $fullImageName -}}
      {{- end -}}
    {{- end -}}
  {{- end -}}
...
{{- else if (or (gt (len $relocatedImages) 0) (gt (len $replacedImages) 0)) -}}
  ...
  {{- range (concat $relocatedImages $replacedImages) -}}
    {{- $errorString = print $errorString "\n  - " . -}}
  {{- end -}}
  {{- if and (eq (len $relocatedImages) 0) (eq (len $replacedImages) (len $bitmoaLegacyImages)) -}}
    {{- $errorString = print "\n\n⚠ WARNING: " $errorString -}}
    {{- print $errorString -}}
  {{- else if or (contains "docker.io/bitmoa/" $originalImages) (contains "docker.io/bitmoaprem/" $originalImages) (contains "docker.io/bitmoasecure/" $originalImages) -}}
    {{- $errorString = print "\n\n⚠ ERROR: " $errorString -}}
    ...
    {{- $errorString = print $errorString "\nFurther information can be obtained at https://github.com/bitmoa/charts/issues/30850" -}}
