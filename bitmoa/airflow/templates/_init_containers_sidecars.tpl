...
- name: prepare-config
  ...
  args:
    ...
    - |
      ...
      cp /opt/bitmoa/airflow/airflow.cfg /emptydir/app-base-dir/airflow.cfg
    ...
    {{- end }}
      ...
      if [[ -f "/opt/bitmoa/airflow/config/airflow_local_settings.py" ]]; then
          cp /opt/bitmoa/airflow/config/airflow_local_settings.py /emptydir/app-conf-dir/airflow_local_settings.py
      else
  ...
  env:
    ...
    - name: AIRFLOW_DATABASE_SQL_CONN_FILE
      value: {{ printf "/opt/bitmoa/airflow/secrets/%s" (include "airflow.database.secretKey" .) }}
    {{- else }}
    ...
    - name: REDIS_PASSWORD_FILE
      value: "/opt/bitmoa/airflow/secrets/redis-password"
    {{- else }}
    - name: REDIS_PASSWORD
      valueFrom:
        secretKeyRef:
  ...
  volumeMounts:
    - name: empty-dir
      mountPath: /emptydir
    - name: configuration
      mountPath: /opt/bitmoa/airflow/airflow.cfg
      subPath: airflow.cfg
    - name: configuration
      mountPath: /opt/bitmoa/airflow/config/airflow_local_settings.py
      subPath: airflow_local_settings.py
    {{- if  .Values.usePasswordFiles }}
    - name: airflow-secrets
      mountPath: /opt/bitmoa/airflow/secrets
    {{- end }}
{{- end -}}
...
- name: prepare-web-config
  ...
  args:
    ...
    - |
      ...
      # Copy the configuration files to the writable directory
      cp /opt/bitmoa/airflow/webserver_config.py /emptydir/app-base-dir/webserver_config.py
  ...
  env:
    ...
    - name: AIRFLOW_LDAP_BIND_PASSWORD_FILE
      value: "/opt/bitmoa/airflow/secrets/bind-password"
    {{- else }}
  ...
  volumeMounts:
    ...
    - name: webserver-configuration
      mountPath: /opt/bitmoa/airflow/webserver_config.py
...
- name: wait-for-db-migrations
  ...
  volumeMounts:
    - name: empty-dir
      ...
      subPath: tmp-dir
    - name: empty-dir
      mountPath: /opt/bitmoa/airflow/logs
      subPath: app-logs-dir
    - name: empty-dir
      mountPath: /opt/bitmoa/airflow/tmp
      subPath: app-tmp-dir
    - name: empty-dir
      mountPath: /opt/bitmoa/airflow/airflow.db
...
- name: prepare-venv
  ...
  args:
    ...
    - |
      ...
      # Copy the configuration files to the writable directory
      cp -r --preserve=mode /opt/bitmoa/airflow/venv /emptydir/venv-base-dir
...
- image: {{ include "airflow.image" . }}
  ...
  volumeMounts:
    - name: empty-dir
      ...
      subPath: tmp-dir
    - name: empty-dir
      mountPath: /opt/bitmoa/airflow/nss-wrapper
      subPath: app-nss-wrapper-dir
    - name: empty-dir
      mountPath: /etc/ssh
      subPath: etc-ssh-dir
    - name: empty-dir
      mountPath: /opt/bitmoa/airflow/.ssh
      subPath: ssh-dir
...
{{ include "airflow.defaultInitContainers.shared" . }}
    ...
    - name: dags-ssh-key
      mountPath: /opt/bitmoa/airflow/.ssh/dags-ssh-key
  ...
  args:
    ...
    {{- if or .Values.dags.sshKey .Values.dags.existingSshKeySecret }}
      export GIT_SSH_COMMAND="ssh -i /opt/bitmoa/airflow/.ssh/dags-ssh-key -o StrictHostKeyChecking=no"
    {{- end }}
...
{{ include "airflow.defaultInitContainers.shared" . }}
    ...
    - name: plugins-ssh-key
      mountPath: /opt/bitmoa/airflow/.ssh/plugins-ssh-key
  ...
  args:
    ...
    {{- if or .Values.plugins.sshKey .Values.plugins.existingSshKeySecret }}
      export GIT_SSH_COMMAND="ssh -i /opt/bitmoa/airflow/.ssh/plugins-ssh-key -o StrictHostKeyChecking=no"
    {{- end }}
...
{{- if not (empty .Values.dags.existingConfigmap) }}
- name: empty-dir
  mountPath: /opt/bitmoa/airflow/dags/external
  subPath: app-dags-dir/external
{{- end }}
{{- range .Values.dags.repositories }}
- name: empty-dir
  mountPath: /opt/bitmoa/airflow/dags/git_{{ include "airflow.dagsPlugins.repository.name" . }}
  {{- if .path }}
...
{{- range .Values.plugins.repositories }}
- name: empty-dir
  mountPath: /opt/bitmoa/airflow/plugins/git_{{ include "airflow.dagsPlugins.repository.name" . }}
  {{- if .path }}
