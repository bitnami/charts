...
- name: prepare-write-dirs
  ...
  args:
    ...
    - |
      ...
      cp -r --preserve=mode /opt/bitmoa/cilium/var/lib/bpf /emptydir/bpf-lib-dir
...
- name: install-cni-plugin
  ...
  {{- end }}
  args:
    - /opt/bitmoa/scripts/cilium/install-cni-plugin.sh
    - /host
  env:
...
- name: host-mount-cgroup2
  ...
  {{- end }}
  args:
    - /opt/bitmoa/scripts/cilium/mount-cgroup2.sh
    - /host
...
- name: clean-state
  ...
  volumeMounts:
    ...
    {{- end }}
    - name: cilium-run
      mountPath: /opt/bitmoa/cilium/var/run
...
- name: wait-for-kube-proxy
  ...
  {{- end }}
  args:
    - /opt/bitmoa/scripts/cilium/wait-for-kube-proxy.sh
{{- end -}}
