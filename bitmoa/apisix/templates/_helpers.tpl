...
- name: prepare-apisix-dirs
  ...
  command:
    ...
    - |
      #!/bin/bash
      cp -R /opt/bitmoa/apisix/conf /usr/local/apisix
      ln -sf /opt/bitmoa/apisix/deps /usr/local/apisix
      ln -sf /opt/bitmoa/apisix/ui /usr/local/apisix
      ln -sf /opt/bitmoa/apisix/openresty/luajit/share/lua/*/apisix /usr/local/apisix
    ...
    {{- end }}
      ...
      cp /bitmoa/apisix/rendered-conf/config.yaml /usr/local/apisix/conf/
      ...
      cp /bitmoa/certs/{{ $block.tls.certFilename }} /usr/local/apisix/conf/cert/ssl_PLACE_HOLDER.crt
      cp /bitmoa/certs/{{ $block.tls.certKeyFilename }} /usr/local/apisix/conf/cert/ssl_PLACE_HOLDER.key
      {{- end }}
  env:
    - name: BITMOA_DEBUG
    ...
    - name: APISIX_ADMIN_API_TOKEN_FILE
      value: {{ printf "/opt/bitmoa/apisix/secrets/%s" (include "apisix.control-plane.adminTokenKey" .context) }}
    - name: APISIX_VIEWER_API_TOKEN_FILE
      value: {{ printf "/opt/bitmoa/apisix/secrets/%s" (include "apisix.control-plane.viewerTokenKey" .context) }}
    {{- else }}
    ...
    - name: APISIX_ETCD_PASSWORD_FILE
      value: {{ printf "/opt/bitmoa/apisix/secrets/%s" (include "apisix.etcd.secretPasswordKey" .context) }}
    {{- else }}
  ...
  volumeMounts:
    - name: empty-dir
      ...
      subPath: app-tmp-dir
    - name: empty-dir
      mountPath: /bitmoa/apisix/rendered-conf
    ...
    - name: apisix-secrets
      mountPath: /opt/bitmoa/apisix/secrets
    {{- end }}
    {{- if $block.tls.enabled }}
    - name: certs
      mountPath: /bitmoa/certs
    {{- end }}
{{- end -}}
...
- name: render-conf
  ...
  command:
    ...
    {{- if .context.Values.usePasswordFiles }}
      ...
      {{- end }}
    {{- end }}
      find /bitmoa/apisix/conf -type f -name *.yaml -print0 | sort -z | xargs -0 yq eval-all '. as $item ireduce ({}; . * $item )' > /bitmoa/apisix/rendered-conf/pre-render-config.yaml
      render-template /bitmoa/apisix/rendered-conf/pre-render-config.yaml > /bitmoa/apisix/rendered-conf/config.yaml
      chmod 644 /bitmoa/apisix/rendered-conf/config.yaml
      rm /bitmoa/apisix/rendered-conf/pre-render-config.yaml
  env:
    - name: BITMOA_DEBUG
      value: {{ ternary "true" "false" (or .context.Values.image.debug .context.Values.diagnosticMode.enabled) | quote }}
    {{- if .context.Values.controlPlane.enabled }}
  ...
  volumeMounts:
    - name: empty-dir
      ...
      subPath: app-conf-dir
    - name: config
      mountPath: /bitmoa/apisix/conf/00_default
    {{- if and .context.Values.usePasswordFiles (or .context.Values.controlPlane.enabled (include "apisix.etcd.authEnabled" .context)) }}
    - name: apisix-secrets
      mountPath: /opt/bitmoa/apisix/secrets
    {{- end }}
    ...
    - name: extra-config
      mountPath: /bitmoa/apisix/conf/01_extra
    {{- end }}
    {{- if $block.tls.enabled }}
    - name: certs
      mountPath: /bitmoa/certs
