...
- name: copy-certs
  ...
  command:
    ...
    - |
      #!/bin/bash
      cp -L /tmp/certs/* /bitmoa/mlflow/certs
...
- name: get-default-auth-conf
  ...
  command:
    ...
    - |
      #!/bin/bash
      cp /bitmoa/mlflow-basic-auth/basic_auth.ini /bitmoa/rendered-basic-auth/basic_auth.ini
  volumeMounts:
    - name: tmp
      mountPath: /tmp
    - name: rendered-basic-auth
      mountPath: /bitmoa/rendered-basic-auth
  {{- if .Values.tracking.resources }}
  resources: {{- toYaml .Values.tracking.resources | nindent 4 }}
  {{- else if ne .Values.tracking.resourcesPreset "none" }}
  resources: {{- include "common.resources.preset" (dict "type" .Values.tracking.resourcesPreset) | nindent 4 }}
...
- name: render-auth-conf
  ...
  command:
    ...
    - |
      ...
      render-template /bitmoa/basic-auth-overrides/*.ini > /tmp/rendered-overrides.ini
      ...
      do
        ...
        ini-file set -s mlflow -k "$key" -v "$value" /bitmoa/rendered-basic-auth/basic_auth.ini
  ...
  volumeMounts:
    ...
    - name: basic-auth-overrides
      mountPath: /bitmoa/basic-auth-overrides
    - name: rendered-basic-auth
      mountPath: /bitmoa/rendered-basic-auth
  {{- if .Values.tracking.resources }}
  resources: {{- toYaml .Values.tracking.resources | nindent 4 }}
...
{{- if .Values.tracking.tls.autoGenerated }}
    {{- printf "/bitmoa/mlflow/certs/tls.crt" -}}
{{- else -}}
    {{- required "Certificate filename is required when TLS in enabled" .Values.tracking.tls.certFilename | printf "/bitmoa/mlflow/certs/%s" -}}
{{- end -}}
{{- end -}}
...
{{- if .Values.tracking.tls.autoGenerated }}
    {{- printf "/bitmoa/mlflow/certs/tls.key" -}}
{{- else -}}
{{- required "Certificate Key filename is required when TLS in enabled" .Values.tracking.tls.certKeyFilename | printf "/bitmoa/mlflow/certs/%s" -}}
{{- end -}}
{{- end -}}
...
{{- if .Values.tracking.tls.autoGenerated }}
    {{- printf "/bitmoa/mlflow/certs/ca.crt" -}}
{{- else -}}
    {{- if .Values.tracking.tls.certCAFilename -}}
    {{- printf "/bitmoa/mlflow/certs/%s" .Values.tracking.tls.certCAFilename -}}
    {{- end -}}
{{- end -}}
{{- end -}}
...
- name: git-clone-repository
  ...
  args:
    ...
    - |
      #!/bin/bash
      rm -rf /app/*
      [[ -f "/opt/bitmoa/scripts/git/entrypoint.sh" ]] && source "/opt/bitmoa/scripts/git/entrypoint.sh"
