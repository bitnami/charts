...
- name: SUPERSET_DATABASE_PASSWORD_FILE
  value: {{ printf "/opt/bitmoa/superset/secrets/%s" (include "superset.database.secretKey" .) }}
{{- else }}
...
- name: REDIS_PASSWORD_FILE
  value: {{ printf "/opt/bitmoa/superset/secrets/%s" (include "superset.redis.secretKey" .) }}
{{- else }}
- name: REDIS_PASSWORD
  valueFrom:
    secretKeyRef:
...
- name: SUPERSET_SECRET_KEY_FILE
  value: "/opt/bitmoa/superset/secrets/superset-secret-key"
{{- else }}
...
- name: SUPERSET_CONF_FILE
  value: "/bitmoa/superset/conf/superset_config.py"
{{- end }}
- name: BITMOA_DEBUG
...
- name: wait-for-db
  ...
  volumeMounts:
    - name: superset-secrets
      mountPath: /opt/bitmoa/superset/secrets
      readOnly: true
  {{- end }}
{{- end -}}
