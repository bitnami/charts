...
- name: wait-for-etcd
    ...
    - |
      ...
      check_etcd() {
          ...
          if echo $etcd_host | grep https; then
             params_cert="--cacert /bitmoa/milvus/conf/cert/etcd/client/{{ .Values.externalEtcd.tls.caCert }} --cert /bitmoa/milvus/conf/cert/etcd/client/{{ .Values.externalEtcd.tls.cert }} --key /bitmoa/milvus/conf/cert/etcd/client/{{ .Values.externalEtcd.tls.key }}"
          fi
  ...
  volumeMounts:
    - name: etcd-client-certs
      mountPath: /bitmoa/milvus/conf/cert/etcd/client
      readOnly: true
  {{- end }}
{{- end -}}
...
- name: wait-for-s3
  ...
  command:
    ...
    - |
      ...
      check_s3() {
          ...
          params_cert="--cacert /bitmoa/milvus/conf/cert/minio/client/{{ .Values.externalS3.tls.caCert }}"
          {{- else }}
          params_cert="-k"
          {{- end }}
          {{- end }}
  ...
  volumeMounts:
    - name: minio-client-certs
      mountPath: /bitmoa/milvus/conf/cert/minio/client
      readOnly: true
  {{- end }}
{{- end -}}
...
- name: copy-default-configuration
  ...
  command:
    ...
    - |
      ...
      cp -r /opt/bitmoa/milvus/configs/. /bitmoa/milvus/rendered-conf
  volumeMounts:
    - name: empty-dir
      mountPath: /bitmoa/milvus/rendered-conf
...
- name: prepare-milvus
  ...
  command:
    ...
    - |
      ...
      find /bitmoa/milvus/conf -type f -name *.yaml -print0 | sort -z | xargs -0 yq eval-all '. as $item ireduce ({}; . * $item )' /bitmoa/milvus/rendered-conf/milvus.yaml > /bitmoa/milvus/rendered-conf/pre-render-config_00.yaml
      ...
      yq 'del(.pulsar)' /bitmoa/milvus/rendered-conf/pre-render-config_00.yaml > /bitmoa/milvus/rendered-conf/pre-render-config_01.yaml
      # Kafka TLS settings
      ...
      yq e -i '.kafka.ssl.enabled = true' /bitmoa/milvus/rendered-conf/pre-render-config_01.yaml
      {{- if and .context.Values.externalKafka.tls.cert .context.Values.externalKafka.tls.key }}
      yq e -i '.kafka.ssl.tlsCert = "/opt/bitmoa/milvus/configs/cert/kafka/client/{{ .context.Values.externalKafka.tls.cert }}"' /bitmoa/milvus/rendered-conf/pre-render-config_01.yaml
      yq e -i '.kafka.ssl.tlsKey = "/opt/bitmoa/milvus/configs/cert/kafka/client/{{ .context.Values.externalKafka.tls.key }}"' /bitmoa/milvus/rendered-conf/pre-render-config_01.yaml
      {{- end }}
      {{- if .context.Values.externalKafka.tls.caCert }}
      yq e -i '.kafka.ssl.tlsCaCert = "/opt/bitmoa/milvus/configs/cert/kafka/client/{{ .context.Values.externalKafka.tls.caCert }}"' /bitmoa/milvus/rendered-conf/pre-render-config_01.yaml
      {{- end }}
      {{- if .context.Values.externalKafka.tls.keyPassword }}
      yq e -i '.kafka.ssl.tlsKeyPassword = {{ print "{{ MILVUS_KAFKA_TLS_KEY_PASSWORD }}" | quote }}' /bitmoa/milvus/rendered-conf/pre-render-config_01.yaml
      {{- end }}
      {{- end }}
      {{- else }}
      mv /bitmoa/milvus/rendered-conf/pre-render-config_00.yaml /bitmoa/milvus/rendered-conf/pre-render-config_01.yaml
      {{- end }}
      ...
      yq e '.minio.ssl.tlsCACert = "/opt/bitmoa/milvus/configs/cert/minio/client/{{ .context.Values.externalS3.tls.caCert }}"' /bitmoa/milvus/rendered-conf/pre-render-config_01.yaml > /bitmoa/milvus/rendered-conf/pre-render-config_02.yaml
      {{- else }}
      yq e '.minio.ssl.tlsCACert = ""' /bitmoa/milvus/rendered-conf/pre-render-config_01.yaml > /bitmoa/milvus/rendered-conf/pre-render-config_02.yaml
      {{- end }}
      {{- else }}
      mv /bitmoa/milvus/rendered-conf/pre-render-config_01.yaml /bitmoa/milvus/rendered-conf/pre-render-config_02.yaml
      {{- end }}

      # Milvus server TLS settings
      yq e '.common.security.tlsMode = {{ .context.Values.proxy.tls.mode }}' /bitmoa/milvus/rendered-conf/pre-render-config_02.yaml > /bitmoa/milvus/rendered-conf/pre-render-config_03.yaml
      {{- if ne (int .context.Values.proxy.tls.mode) 0 }}
      yq e -i '.tls.serverPemPath = "/opt/bitmoa/milvus/configs/cert/milvus/{{ .context.Values.proxy.tls.cert }}"' /bitmoa/milvus/rendered-conf/pre-render-config_03.yaml
      yq e -i '.tls.serverKeyPath = "/opt/bitmoa/milvus/configs/cert/milvus/{{ .context.Values.proxy.tls.key }}"' /bitmoa/milvus/rendered-conf/pre-render-config_03.yaml
      {{- if eq (int .context.Values.proxy.tls.mode) 2 }}
      yq e -i '.tls.caPemPath = "/opt/bitmoa/milvus/configs/cert/milvus/{{ .context.Values.proxy.tls.caCert }}"' /bitmoa/milvus/rendered-conf/pre-render-config_03.yaml
      {{- end }}
      {{- end }}

      render-template /bitmoa/milvus/rendered-conf/pre-render-config_03.yaml > /bitmoa/milvus/rendered-conf/milvus.yaml
      rm /bitmoa/milvus/rendered-conf/pre-render-config*
      chmod 644 /bitmoa/milvus/rendered-conf/milvus.yaml
  env:
    - name: BITMOA_DEBUG
    ...
    - name: MILVUS_KAFKA_PASSWORD_FILE
      value: {{ printf "/opt/bitmoa/milvus/secrets/%s" (include "milvus.kafka.secretPasswordKey" .context) }}
    {{- else }}
    ...
    - name: MILVUS_KAFKA_TLS_KEY_PASSWORD_FILE
      value: "/opt/bitmoa/milvus/secrets/key-password"
    {{- else }}
    ...
    - name: MILVUS_S3_ACCESS_ID_FILE
      value: {{ printf "/opt/bitmoa/milvus/secrets/%s" (include "milvus.s3.accessKeyIDKey" .context) }}
    - name: MILVUS_S3_SECRET_ACCESS_KEY_FILE
      value: {{ printf "/opt/bitmoa/milvus/secrets/%s" (include "milvus.s3.secretAccessKeyKey" .context) }}
    {{- else }}
  ...
  volumeMounts:
    - name: config-common
      mountPath: /bitmoa/milvus/conf/00_default
