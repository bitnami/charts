...
| `credentialsFile.enabled`            | Use credential files in stead of env vars                                                                                                     | `false`                                                 |
| `credentialsFile.useSecretValues`    | Use the values from a Kubernetes secret                                                                                                   | `true`                                                  |
| `credentialsFile.mountPath`          | The path to mount the credentials secret. (when credentialsFile.useSecretValues is false this is the path where the self provisioned files are located)                                                                           | `/opt/bitnami/minio/secrets/`                           |
| `credentialsFile.accessKeyFile`      | File name for the access key file                               | `access-key`                                            |
| `credentialsFile.secretKeyFile`      | File name for the secret key file                               | `secret-key`                                            |
