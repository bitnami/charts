...
| `secretKeys.adminPasswordKey`  | Name of key in existing secret to use for Tomcat admin password (overrides `tomcatPassword`). Only used when `existingSecret` is set.                                                            | `""`                     |
