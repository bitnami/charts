...
| `existingSecret`               | Use existing secret for credentials details (`tomcatPassword` will be ignored and picked up from this secret. `tomcatUsername` might be ignored depending of provided `adminUsernameKey` value). | `""`                     |
| `secretKeys.adminUsernameKey`  | Name of key in existing secret to use for Tomcat admin username (overrides `tomcatUsername` if not empty). Only used when `existingSecret` is set.                                               | `""`                     |
| `secretKeys.adminPasswordKey`  | Name of key in existing secret to use for Tomcat admin password (overrides `tomcatPassword`). Only used when `existingSecret` is set.                                                            | `tomcat-password`        |
