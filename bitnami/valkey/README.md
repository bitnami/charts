...
# Bitnami package for Valkey(R)

Valkey(R) is an open source, advanced key-value store. It is often referred to as a data structure server since keys can contain strings, hashes, lists, sets and sorted sets.

[Overview of Valkey&reg;](http://valkey.io)
...
Looking to use Valkey&reg; in production? Try [VMware Tanzu Application Catalog](https://bitnami.com/enterprise), the enterprise edition of Bitnami Application Catalog.

## Introduction

This chart bootstraps a [Valkey&reg;](https://github.com/bitnami/containers/tree/main/bitnami/valkey) deployment on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.

Bitnami charts can be used with [Kubeapps](https://kubeapps.dev/) for deployment and management of Helm Charts in clusters.

### Choose between Valkey&reg; Helm Chart and Valkey&reg; Cluster Helm Chart

You can choose any of the two Valkey&reg; Helm charts for deploying a Valkey&reg; cluster.

1. [Valkey&reg; Helm Chart](https://github.com/bitnami/charts/tree/main/bitnami/valkey) will deploy a master-replica cluster, with the [option](https://github.com/bitnami/charts/tree/main/bitnami/valkey#valkey-sentinel-configuration-parameters) of enabling using Valkey&reg; Sentinel.
2. [Valkey&reg; Cluster Helm Chart](https://github.com/bitnami/charts/tree/main/bitnami/valkey-cluster) will deploy a Valkey&reg; Cluster topology with sharding.

The main features of each chart are the following:

| Valkey&reg;                                     | Valkey&reg; Cluster                                             |
|--------------------------------------------------------|------------------------------------------------------------------------|
| Supports multiple databases                            | Supports only one database. Better if you have a big dataset           |
| Single write point (single master)                     | Multiple write points (multiple masters)                               |
| ![Valkey&reg; Topology](img/valkey-topology.png) | ![Valkey&reg; Cluster Topology](img/valkey-cluster-topology.png) |
...
The command deploys Valkey&reg; on the Kubernetes cluster in the default configuration. The [Parameters](#parameters) section lists the parameters that can be configured during installation.
...
### Use a different Valkey&reg; version
...
When installing the chart with `architecture=replication`, it will deploy a Valkey&reg; master StatefulSet and a Valkey&reg; replicas StatefulSet. The replicas will be read-replicas of the master. Two services will be exposed:

- Valkey&reg; Master service: Points to the master, where read-write operations can be performed
- Valkey&reg; Replicas service: Points to the replicas, where only read operations are allowed by default.
...
When installing the chart with `architecture=standalone`, it will deploy a standalone Valkey&reg; StatefulSet. A single service will be exposed:

- Valkey&reg; Master service: Points to the master, where read-write operations can be performed

#### Master-Replicas with Sentinel

When installing the chart with `architecture=replication` and `sentinel.enabled=true`, it will deploy a Valkey&reg; master StatefulSet (only one master allowed) and a Valkey&reg; replicas StatefulSet. In this case, the pods will contain an extra container with Valkey&reg; Sentinel. This container will form a cluster of Valkey&reg; Sentinel nodes, which will promote a new master in case the actual one fails.

On graceful termination of the Valkey&reg; master pod, a failover of the master is initiated to promote a new master. The Valkey&reg; Sentinel container in this pod will wait for the failover to occur before terminating. If `sentinel.valkeyShutdownWaitFailover=true` is set (the default), the Valkey&reg; container will wait for the failover as well before terminating. This increases availability for reads during failover, but may cause stale reads until all clients have switched to the new master.

In addition to this, only one service is exposed:

- Valkey&reg; service: Exposes port 6379 for Valkey&reg; read-only operations and port 26379 for accessing Valkey&reg; Sentinel.

For read-only operations, access the service using port 6379. For write operations, it's necessary to access the Valkey&reg; Sentinel cluster and query the current master using the command below (using valkey-cli or similar):
...
To use a password file for Valkey&reg; you need to create a secret containing the password and then deploy the chart using that secret. Follow these instructions:
...
A custom Lua script can be added to the `valkey-exporter` sidecar by way of the `metrics.extraArgs.script` parameter.  The pathname of the script must exist on the container, or the `valkey_exporter` process (and therefore the whole pod) will refuse to start.  The script can be provided to the sidecar containers via the `metrics.extraVolumes` and `metrics.extraVolumeMounts` parameters:
...
Valkey&reg; may require some changes in the kernel of the host machine to work as expected, in particular increasing the `somaxconn` value and disabling transparent huge pages. To do so, you can set up a privileged `initContainer` with the `sysctlImage` config values, for example:
...
To enable network policy for Valkey&reg;, install [a networking plugin that implements the Kubernetes NetworkPolicy spec](https://kubernetes.io/docs/tasks/administer-cluster/declare-network-policy#before-you-begin), and set `networkPolicy.enabled` to `true`.
...
| `global.valkey.password`                               | Global Valkey&reg; password (overrides `auth.password`)                                                                                                                                                                                                                                                                                                              | `""`   |
...
### Valkey&reg; Image parameters

| Name                | Description                                                                                                | Value                   |
| ------------------- | ---------------------------------------------------------------------------------------------------------- | ----------------------- |
| `image.registry`    | Valkey&reg; image registry                                                                                  | `REGISTRY_NAME`         |
| `image.repository`  | Valkey&reg; image repository                                                                                | `REPOSITORY_NAME/valkey` |
| `image.digest`      | Valkey&reg; image digest in the way sha256:aa.... Please note this parameter, if set, will override the tag | `""`                    |
| `image.pullPolicy`  | Valkey&reg; image pull policy                                                                               | `IfNotPresent`          |
| `image.pullSecrets` | Valkey&reg; image pull secrets                                                                              | `[]`                    |
| `image.debug`       | Enable image debug mode                                                                                    | `false`                 |

### Valkey&reg; common configuration parameters

| Name                             | Description                                                                           | Value         |
| -------------------------------- | ------------------------------------------------------------------------------------- | ------------- |
| `architecture`                   | Valkey&reg; architecture. Allowed values: `standalone` or `replication`                | `replication` |
| `auth.enabled`                   | Enable password authentication                                                        | `true`        |
| `auth.sentinel`                  | Enable password authentication on sentinels too                                       | `true`        |
| `auth.password`                  | Valkey&reg; password                                                                   | `""`          |
| `auth.existingSecret`            | The name of an existing secret with Valkey&reg; credentials                            | `""`          |
| `auth.existingSecretPasswordKey` | Password key to be retrieved from existing secret                                     | `""`          |
| `auth.usePasswordFiles`          | Mount credentials as files instead of using an environment variable                   | `false`       |
| `auth.usePasswordFileFromSecret` | Mount password file from secret                                                       | `true`        |
| `commonConfiguration`            | Common configuration to be added into the ConfigMap                                   | `""`          |
| `existingConfigmap`              | The name of an existing ConfigMap with your custom configuration for Valkey&reg; nodes | `""`          |

### Valkey&reg; master configuration parameters

| Name                                                       | Description                                                                                                                                                                                                                     | Value                    |
| ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ |
| `master.count`                                             | Number of Valkey&reg; master instances to deploy (experimental, requires additional configuration)                                                                                                                               | `1`                      |
| `master.configuration`                                     | Configuration for Valkey&reg; master nodes                                                                                                                                                                                       | `""`                     |
| `master.disableCommands`                                   | Array with Valkey&reg; commands to disable on master nodes                                                                                                                                                                       | `["FLUSHDB","FLUSHALL"]` |
...
| `master.preExecCmds`                                       | Additional commands to run prior to starting Valkey&reg; master                                                                                                                                                                  | `[]`                     |
| `master.extraFlags`                                        | Array with additional command line flags for Valkey&reg; master                                                                                                                                                                  | `[]`                     |
| `master.extraEnvVars`                                      | Array with extra environment variables to add to Valkey&reg; master nodes                                                                                                                                                        | `[]`                     |
| `master.extraEnvVarsCM`                                    | Name of existing ConfigMap containing extra env vars for Valkey&reg; master nodes                                                                                                                                                | `""`                     |
| `master.extraEnvVarsSecret`                                | Name of existing Secret containing extra env vars for Valkey&reg; master nodes                                                                                                                                                   | `""`                     |
| `master.containerPorts.valkey`                              | Container port to open on Valkey&reg; master nodes                                                                                                                                                                               | `6379`                   |
| `master.startupProbe.enabled`                              | Enable startupProbe on Valkey&reg; master nodes                                                                                                                                                                                  | `false`                  |
...
| `master.livenessProbe.enabled`                             | Enable livenessProbe on Valkey&reg; master nodes                                                                                                                                                                                 | `true`                   |
...
| `master.readinessProbe.enabled`                            | Enable readinessProbe on Valkey&reg; master nodes                                                                                                                                                                                | `true`                   |
...
| `master.podSecurityContext.enabled`                        | Enabled Valkey&reg; master pods' Security Context                                                                                                                                                                                | `true`                   |
...
| `master.podSecurityContext.fsGroup`                        | Set Valkey&reg; master pod's Security Context fsGroup                                                                                                                                                                            | `1001`                   |
| `master.containerSecurityContext.enabled`                  | Enabled Valkey&reg; master containers' Security Context                                                                                                                                                                          | `true`                   |
| `master.containerSecurityContext.seLinuxOptions`           | Set SELinux options in container                                                                                                                                                                                                | `{}`                     |
| `master.containerSecurityContext.runAsUser`                | Set Valkey&reg; master containers' Security Context runAsUser                                                                                                                                                                    | `1001`                   |
| `master.containerSecurityContext.runAsGroup`               | Set Valkey&reg; master containers' Security Context runAsGroup                                                                                                                                                                   | `1001`                   |
| `master.containerSecurityContext.runAsNonRoot`             | Set Valkey&reg; master containers' Security Context runAsNonRoot                                                                                                                                                                 | `true`                   |
| `master.containerSecurityContext.allowPrivilegeEscalation` | Is it possible to escalate Valkey&reg; pod(s) privileges                                                                                                                                                                         | `false`                  |
| `master.containerSecurityContext.readOnlyRootFilesystem`   | Set container's Security Context read-only root filesystem                                                                                                                                                                      | `true`                   |
| `master.containerSecurityContext.seccompProfile.type`      | Set Valkey&reg; master containers' Security Context seccompProfile                                                                                                                                                               | `RuntimeDefault`         |
| `master.containerSecurityContext.capabilities.drop`        | Set Valkey&reg; master containers' Security Context capabilities to drop                                                                                                                                                         | `["ALL"]`                |
| `master.kind`                                              | Use either Deployment, StatefulSet (default) or DaemonSet                                                                                                                                                                       | `StatefulSet`            |
| `master.schedulerName`                                     | Alternate scheduler for Valkey&reg; master pods                                                                                                                                                                                  | `""`                     |
| `master.updateStrategy.type`                               | Valkey&reg; master statefulset strategy type                                                                                                                                                                                     | `RollingUpdate`          |
| `master.minReadySeconds`                                   | How many seconds a pod needs to be ready before killing the next, during update                                                                                                                                                 | `0`                      |
| `master.priorityClassName`                                 | Valkey&reg; master pods' priorityClassName                                                                                                                                                                                       | `""`                     |
| `master.automountServiceAccountToken`                      | Mount Service Account token in pod                                                                                                                                                                                              | `false`                  |
| `master.hostAliases`                                       | Valkey&reg; master pods host aliases                                                                                                                                                                                             | `[]`                     |
| `master.podLabels`                                         | Extra labels for Valkey&reg; master pods                                                                                                                                                                                         | `{}`                     |
| `master.podAnnotations`                                    | Annotations for Valkey&reg; master pods                                                                                                                                                                                          | `{}`                     |
| `master.shareProcessNamespace`                             | Share a single process namespace between all of the containers in Valkey&reg; master pods                                                                                                                                        | `false`                  |
...
| `master.affinity`                                          | Affinity for Valkey&reg; master pods assignment                                                                                                                                                                                  | `{}`                     |
| `master.nodeSelector`                                      | Node labels for Valkey&reg; master pods assignment                                                                                                                                                                               | `{}`                     |
| `master.tolerations`                                       | Tolerations for Valkey&reg; master pods assignment                                                                                                                                                                               | `[]`                     |
| `master.topologySpreadConstraints`                         | Spread Constraints for Valkey&reg; master pod assignment                                                                                                                                                                         | `[]`                     |
| `master.dnsPolicy`                                         | DNS Policy for Valkey&reg; master pod                                                                                                                                                                                            | `""`                     |
| `master.dnsConfig`                                         | DNS Configuration for Valkey&reg; master pod                                                                                                                                                                                     | `{}`                     |
| `master.lifecycleHooks`                                    | for the Valkey&reg; master container(s) to automate configuration before or after startup                                                                                                                                        | `{}`                     |
| `master.extraVolumes`                                      | Optionally specify extra list of additional volumes for the Valkey&reg; master pod(s)                                                                                                                                            | `[]`                     |
| `master.extraVolumeMounts`                                 | Optionally specify extra list of additional volumeMounts for the Valkey&reg; master container(s)                                                                                                                                 | `[]`                     |
| `master.sidecars`                                          | Add additional sidecar containers to the Valkey&reg; master pod(s)                                                                                                                                                               | `[]`                     |
| `master.initContainers`                                    | Add additional init containers to the Valkey&reg; master pod(s)                                                                                                                                                                  | `[]`                     |
| `master.persistence.enabled`                               | Enable persistence on Valkey&reg; master nodes using Persistent Volume Claims                                                                                                                                                    | `true`                   |
| `master.persistence.medium`                                | Provide a medium for `emptyDir` volumes.                                                                                                                                                                                        | `""`                     |
| `master.persistence.sizeLimit`                             | Set this to enable a size limit for `emptyDir` volumes.                                                                                                                                                                         | `""`                     |
| `master.persistence.path`                                  | The path the volume will be mounted at on Valkey&reg; master containers                                                                                                                                                          | `/data`                  |
| `master.persistence.subPath`                               | The subdirectory of the volume to mount on Valkey&reg; master containers                                                                                                                                                         | `""`                     |
| `master.persistence.subPathExpr`                           | Used to construct the subPath subdirectory of the volume to mount on Valkey&reg; master containers                                                                                                                               | `""`                     |
...
| `master.service.type`                                      | Valkey&reg; master service type                                                                                                                                                                                                  | `ClusterIP`              |
| `master.service.ports.valkey`                               | Valkey&reg; master service port                                                                                                                                                                                                  | `6379`                   |
| `master.service.nodePorts.valkey`                           | Node port for Valkey&reg; master                                                                                                                                                                                                 | `""`                     |
| `master.service.externalTrafficPolicy`                     | Valkey&reg; master service external traffic policy                                                                                                                                                                               | `Cluster`                |
| `master.service.extraPorts`                                | Extra ports to expose (normally used with the `sidecar` value)                                                                                                                                                                  | `[]`                     |
| `master.service.internalTrafficPolicy`                     | Valkey&reg; master service internal traffic policy (requires Kubernetes v1.22 or greater to be usable)                                                                                                                           | `Cluster`                |
| `master.service.clusterIP`                                 | Valkey&reg; master service Cluster IP                                                                                                                                                                                            | `""`                     |
| `master.service.loadBalancerIP`                            | Valkey&reg; master service Load Balancer IP                                                                                                                                                                                      | `""`                     |
...
| `master.service.loadBalancerSourceRanges`                  | Valkey&reg; master service Load Balancer sources                                                                                                                                                                                 | `[]`                     |
| `master.service.externalIPs`                               | Valkey&reg; master service External IPs                                                                                                                                                                                          | `[]`                     |
| `master.service.annotations`                               | Additional custom annotations for Valkey&reg; master service                                                                                                                                                                     | `{}`                     |
...
### Valkey&reg; replicas configuration parameters

| Name                                                        | Description                                                                                                                                                                                                                       | Value                    |
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ |
| `replica.kind`                                              | Use either DaemonSet or StatefulSet (default)                                                                                                                                                                                     | `StatefulSet`            |
| `replica.replicaCount`                                      | Number of Valkey&reg; replicas to deploy                                                                                                                                                                                           | `3`                      |
| `replica.configuration`                                     | Configuration for Valkey&reg; replicas nodes                                                                                                                                                                                       | `""`                     |
| `replica.disableCommands`                                   | Array with Valkey&reg; commands to disable on replicas nodes                                                                                                                                                                       | `["FLUSHDB","FLUSHALL"]` |
...
| `replica.preExecCmds`                                       | Additional commands to run prior to starting Valkey&reg; replicas                                                                                                                                                                  | `[]`                     |
| `replica.extraFlags`                                        | Array with additional command line flags for Valkey&reg; replicas                                                                                                                                                                  | `[]`                     |
| `replica.extraEnvVars`                                      | Array with extra environment variables to add to Valkey&reg; replicas nodes                                                                                                                                                        | `[]`                     |
| `replica.extraEnvVarsCM`                                    | Name of existing ConfigMap containing extra env vars for Valkey&reg; replicas nodes                                                                                                                                                | `""`                     |
| `replica.extraEnvVarsSecret`                                | Name of existing Secret containing extra env vars for Valkey&reg; replicas nodes                                                                                                                                                   | `""`                     |
...
| `replica.containerPorts.valkey`                              | Container port to open on Valkey&reg; replicas nodes                                                                                                                                                                               | `6379`                   |
| `replica.startupProbe.enabled`                              | Enable startupProbe on Valkey&reg; replicas nodes                                                                                                                                                                                  | `true`                   |
...
| `replica.livenessProbe.enabled`                             | Enable livenessProbe on Valkey&reg; replicas nodes                                                                                                                                                                                 | `true`                   |
...
| `replica.readinessProbe.enabled`                            | Enable readinessProbe on Valkey&reg; replicas nodes                                                                                                                                                                                | `true`                   |
...
| `replica.podSecurityContext.enabled`                        | Enabled Valkey&reg; replicas pods' Security Context                                                                                                                                                                                | `true`                   |
...
| `replica.podSecurityContext.fsGroup`                        | Set Valkey&reg; replicas pod's Security Context fsGroup                                                                                                                                                                            | `1001`                   |
| `replica.containerSecurityContext.enabled`                  | Enabled Valkey&reg; replicas containers' Security Context                                                                                                                                                                          | `true`                   |
| `replica.containerSecurityContext.seLinuxOptions`           | Set SELinux options in container                                                                                                                                                                                                  | `{}`                     |
| `replica.containerSecurityContext.runAsUser`                | Set Valkey&reg; replicas containers' Security Context runAsUser                                                                                                                                                                    | `1001`                   |
| `replica.containerSecurityContext.runAsGroup`               | Set Valkey&reg; replicas containers' Security Context runAsGroup                                                                                                                                                                   | `1001`                   |
| `replica.containerSecurityContext.runAsNonRoot`             | Set Valkey&reg; replicas containers' Security Context runAsNonRoot                                                                                                                                                                 | `true`                   |
| `replica.containerSecurityContext.allowPrivilegeEscalation` | Set Valkey&reg; replicas pod's Security Context allowPrivilegeEscalation                                                                                                                                                           | `false`                  |
| `replica.containerSecurityContext.readOnlyRootFilesystem`   | Set container's Security Context read-only root filesystem                                                                                                                                                                        | `true`                   |
| `replica.containerSecurityContext.seccompProfile.type`      | Set Valkey&reg; replicas containers' Security Context seccompProfile                                                                                                                                                               | `RuntimeDefault`         |
| `replica.containerSecurityContext.capabilities.drop`        | Set Valkey&reg; replicas containers' Security Context capabilities to drop                                                                                                                                                         | `["ALL"]`                |
| `replica.schedulerName`                                     | Alternate scheduler for Valkey&reg; replicas pods                                                                                                                                                                                  | `""`                     |
| `replica.updateStrategy.type`                               | Valkey&reg; replicas statefulset strategy type                                                                                                                                                                                     | `RollingUpdate`          |
| `replica.minReadySeconds`                                   | How many seconds a pod needs to be ready before killing the next, during update                                                                                                                                                   | `0`                      |
| `replica.priorityClassName`                                 | Valkey&reg; replicas pods' priorityClassName                                                                                                                                                                                       | `""`                     |
...
| `replica.hostAliases`                                       | Valkey&reg; replicas pods host aliases                                                                                                                                                                                             | `[]`                     |
| `replica.podLabels`                                         | Extra labels for Valkey&reg; replicas pods                                                                                                                                                                                         | `{}`                     |
| `replica.podAnnotations`                                    | Annotations for Valkey&reg; replicas pods                                                                                                                                                                                          | `{}`                     |
| `replica.shareProcessNamespace`                             | Share a single process namespace between all of the containers in Valkey&reg; replicas pods                                                                                                                                        | `false`                  |
...
| `replica.affinity`                                          | Affinity for Valkey&reg; replicas pods assignment                                                                                                                                                                                  | `{}`                     |
| `replica.nodeSelector`                                      | Node labels for Valkey&reg; replicas pods assignment                                                                                                                                                                               | `{}`                     |
| `replica.tolerations`                                       | Tolerations for Valkey&reg; replicas pods assignment                                                                                                                                                                               | `[]`                     |
| `replica.topologySpreadConstraints`                         | Spread Constraints for Valkey&reg; replicas pod assignment                                                                                                                                                                         | `[]`                     |
| `replica.dnsPolicy`                                         | DNS Policy for Valkey&reg; replica pods                                                                                                                                                                                            | `""`                     |
| `replica.dnsConfig`                                         | DNS Configuration for Valkey&reg; replica pods                                                                                                                                                                                     | `{}`                     |
| `replica.lifecycleHooks`                                    | for the Valkey&reg; replica container(s) to automate configuration before or after startup                                                                                                                                         | `{}`                     |
| `replica.extraVolumes`                                      | Optionally specify extra list of additional volumes for the Valkey&reg; replicas pod(s)                                                                                                                                            | `[]`                     |
| `replica.extraVolumeMounts`                                 | Optionally specify extra list of additional volumeMounts for the Valkey&reg; replicas container(s)                                                                                                                                 | `[]`                     |
| `replica.sidecars`                                          | Add additional sidecar containers to the Valkey&reg; replicas pod(s)                                                                                                                                                               | `[]`                     |
| `replica.initContainers`                                    | Add additional init containers to the Valkey&reg; replicas pod(s)                                                                                                                                                                  | `[]`                     |
| `replica.persistence.enabled`                               | Enable persistence on Valkey&reg; replicas nodes using Persistent Volume Claims                                                                                                                                                    | `true`                   |
...
| `replica.persistence.path`                                  | The path the volume will be mounted at on Valkey&reg; replicas containers                                                                                                                                                          | `/data`                  |
| `replica.persistence.subPath`                               | The subdirectory of the volume to mount on Valkey&reg; replicas containers                                                                                                                                                         | `""`                     |
| `replica.persistence.subPathExpr`                           | Used to construct the subPath subdirectory of the volume to mount on Valkey&reg; replicas containers                                                                                                                               | `""`                     |
...
| `replica.service.type`                                      | Valkey&reg; replicas service type                                                                                                                                                                                                  | `ClusterIP`              |
| `replica.service.ports.valkey`                               | Valkey&reg; replicas service port                                                                                                                                                                                                  | `6379`                   |
| `replica.service.nodePorts.valkey`                           | Node port for Valkey&reg; replicas                                                                                                                                                                                                 | `""`                     |
| `replica.service.externalTrafficPolicy`                     | Valkey&reg; replicas service external traffic policy                                                                                                                                                                               | `Cluster`                |
| `replica.service.internalTrafficPolicy`                     | Valkey&reg; replicas service internal traffic policy (requires Kubernetes v1.22 or greater to be usable)                                                                                                                           | `Cluster`                |
| `replica.service.extraPorts`                                | Extra ports to expose (normally used with the `sidecar` value)                                                                                                                                                                    | `[]`                     |
| `replica.service.clusterIP`                                 | Valkey&reg; replicas service Cluster IP                                                                                                                                                                                            | `""`                     |
| `replica.service.loadBalancerIP`                            | Valkey&reg; replicas service Load Balancer IP                                                                                                                                                                                      | `""`                     |
...
| `replica.service.loadBalancerSourceRanges`                  | Valkey&reg; replicas service Load Balancer sources                                                                                                                                                                                 | `[]`                     |
| `replica.service.annotations`                               | Additional custom annotations for Valkey&reg; replicas service                                                                                                                                                                     | `{}`                     |
...
### Valkey&reg; Sentinel configuration parameters

| Name                                                         | Description                                                                                                                                                                                                                         | Value                            |
| ------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| `sentinel.enabled`                                           | Use Valkey&reg; Sentinel on Valkey&reg; pods.                                                                                                                                                                                         | `false`                          |
| `sentinel.image.registry`                                    | Valkey&reg; Sentinel image registry                                                                                                                                                                                                  | `REGISTRY_NAME`                  |
| `sentinel.image.repository`                                  | Valkey&reg; Sentinel image repository                                                                                                                                                                                                | `REPOSITORY_NAME/valkey-sentinel` |
| `sentinel.image.digest`                                      | Valkey&reg; Sentinel image digest in the way sha256:aa.... Please note this parameter, if set, will override the tag                                                                                                                 | `""`                             |
| `sentinel.image.pullPolicy`                                  | Valkey&reg; Sentinel image pull policy                                                                                                                                                                                               | `IfNotPresent`                   |
| `sentinel.image.pullSecrets`                                 | Valkey&reg; Sentinel image pull secrets                                                                                                                                                                                              | `[]`                             |
| `sentinel.image.debug`                                       | Enable image debug mode                                                                                                                                                                                                             | `false`                          |
| `sentinel.annotations`                                       | Additional custom annotations for Valkey&reg; Sentinel resource                                                                                                                                                                      | `{}`                             |
...
| `sentinel.valkeyShutdownWaitFailover`                         | Whether the Valkey&reg; master container waits for the failover at shutdown (in addition to the Valkey&reg; Sentinel container).                                                                                                      | `true`                           |
| `sentinel.downAfterMilliseconds`                             | Timeout for detecting a Valkey&reg; node is down                                                                                                                                                                                     | `60000`                          |
...
| `sentinel.configuration`                                     | Configuration for Valkey&reg; Sentinel nodes                                                                                                                                                                                         | `""`                             |
...
| `sentinel.preExecCmds`                                       | Additional commands to run prior to starting Valkey&reg; Sentinel                                                                                                                                                                    | `[]`                             |
| `sentinel.extraEnvVars`                                      | Array with extra environment variables to add to Valkey&reg; Sentinel nodes                                                                                                                                                          | `[]`                             |
| `sentinel.extraEnvVarsCM`                                    | Name of existing ConfigMap containing extra env vars for Valkey&reg; Sentinel nodes                                                                                                                                                  | `""`                             |
| `sentinel.extraEnvVarsSecret`                                | Name of existing Secret containing extra env vars for Valkey&reg; Sentinel nodes                                                                                                                                                     | `""`                             |
...
| `sentinel.containerPorts.sentinel`                           | Container port to open on Valkey&reg; Sentinel nodes                                                                                                                                                                                 | `26379`                          |
| `sentinel.startupProbe.enabled`                              | Enable startupProbe on Valkey&reg; Sentinel nodes                                                                                                                                                                                    | `true`                           |
...
| `sentinel.livenessProbe.enabled`                             | Enable livenessProbe on Valkey&reg; Sentinel nodes                                                                                                                                                                                   | `true`                           |
...
| `sentinel.readinessProbe.enabled`                            | Enable readinessProbe on Valkey&reg; Sentinel nodes                                                                                                                                                                                  | `true`                           |
...
| `sentinel.persistence.enabled`                               | Enable persistence on Valkey&reg; sentinel nodes using Persistent Volume Claims (Experimental)                                                                                                                                       | `false`                          |
...
| `sentinel.containerSecurityContext.enabled`                  | Enabled Valkey&reg; Sentinel containers' Security Context                                                                                                                                                                            | `true`                           |
| `sentinel.containerSecurityContext.seLinuxOptions`           | Set SELinux options in container                                                                                                                                                                                                    | `{}`                             |
| `sentinel.containerSecurityContext.runAsUser`                | Set Valkey&reg; Sentinel containers' Security Context runAsUser                                                                                                                                                                      | `1001`                           |
| `sentinel.containerSecurityContext.runAsGroup`               | Set Valkey&reg; Sentinel containers' Security Context runAsGroup                                                                                                                                                                     | `1001`                           |
| `sentinel.containerSecurityContext.runAsNonRoot`             | Set Valkey&reg; Sentinel containers' Security Context runAsNonRoot                                                                                                                                                                   | `true`                           |
| `sentinel.containerSecurityContext.readOnlyRootFilesystem`   | Set container's Security Context read-only root filesystem                                                                                                                                                                          | `true`                           |
| `sentinel.containerSecurityContext.allowPrivilegeEscalation` | Set Valkey&reg; Sentinel containers' Security Context allowPrivilegeEscalation                                                                                                                                                       | `false`                          |
| `sentinel.containerSecurityContext.seccompProfile.type`      | Set Valkey&reg; Sentinel containers' Security Context seccompProfile                                                                                                                                                                 | `RuntimeDefault`                 |
| `sentinel.containerSecurityContext.capabilities.drop`        | Set Valkey&reg; Sentinel containers' Security Context capabilities to drop                                                                                                                                                           | `["ALL"]`                        |
| `sentinel.lifecycleHooks`                                    | for the Valkey&reg; sentinel container(s) to automate configuration before or after startup                                                                                                                                          | `{}`                             |
| `sentinel.extraVolumes`                                      | Optionally specify extra list of additional volumes for the Valkey&reg; Sentinel                                                                                                                                                     | `[]`                             |
| `sentinel.extraVolumeMounts`                                 | Optionally specify extra list of additional volumeMounts for the Valkey&reg; Sentinel container(s)                                                                                                                                   | `[]`                             |
| `sentinel.service.type`                                      | Valkey&reg; Sentinel service type                                                                                                                                                                                                    | `ClusterIP`                      |
| `sentinel.service.ports.valkey`                               | Valkey&reg; service port for Valkey&reg;                                                                                                                                                                                              | `6379`                           |
| `sentinel.service.ports.sentinel`                            | Valkey&reg; service port for Valkey&reg; Sentinel                                                                                                                                                                                     | `26379`                          |
| `sentinel.service.nodePorts.valkey`                           | Node port for Valkey&reg;                                                                                                                                                                                                            | `""`                             |
| `sentinel.service.nodePorts.sentinel`                        | Node port for Sentinel                                                                                                                                                                                                              | `""`                             |
| `sentinel.service.externalTrafficPolicy`                     | Valkey&reg; Sentinel service external traffic policy                                                                                                                                                                                 | `Cluster`                        |
| `sentinel.service.extraPorts`                                | Extra ports to expose (normally used with the `sidecar` value)                                                                                                                                                                      | `[]`                             |
| `sentinel.service.clusterIP`                                 | Valkey&reg; Sentinel service Cluster IP                                                                                                                                                                                              | `""`                             |
| `sentinel.service.createMaster`                              | Enable master service pointing to the current master (experimental)                                                                                                                                                                 | `false`                          |
| `sentinel.service.loadBalancerIP`                            | Valkey&reg; Sentinel service Load Balancer IP                                                                                                                                                                                        | `""`                             |
...
| `sentinel.service.loadBalancerSourceRanges`                  | Valkey&reg; Sentinel service Load Balancer sources                                                                                                                                                                                   | `[]`                             |
| `sentinel.service.annotations`                               | Additional custom annotations for Valkey&reg; Sentinel service                                                                                                                                                                       | `{}`                             |
...
| Name                                                        | Description                                                                                                                                                                                                                       | Value                            |
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| `metrics.enabled`                                           | Start a sidecar prometheus exporter to expose Valkey&reg; metrics                                                                                                                                                                  | `false`                          |
| `metrics.image.registry`                                    | Valkey&reg; Exporter image registry                                                                                                                                                                                                | `REGISTRY_NAME`                  |
| `metrics.image.repository`                                  | Valkey&reg; Exporter image repository                                                                                                                                                                                              | `REPOSITORY_NAME/valkey-exporter` |
| `metrics.image.digest`                                      | Valkey&reg; Exporter image digest in the way sha256:aa.... Please note this parameter, if set, will override the tag                                                                                                               | `""`                             |
| `metrics.image.pullPolicy`                                  | Valkey&reg; Exporter image pull policy                                                                                                                                                                                             | `IfNotPresent`                   |
| `metrics.image.pullSecrets`                                 | Valkey&reg; Exporter image pull secrets                                                                                                                                                                                            | `[]`                             |
| `metrics.containerPorts.http`                               | Metrics HTTP container port                                                                                                                                                                                                       | `9121`                           |
| `metrics.startupProbe.enabled`                              | Enable startupProbe on Valkey&reg; replicas nodes                                                                                                                                                                                  | `false`                          |
...
| `metrics.livenessProbe.enabled`                             | Enable livenessProbe on Valkey&reg; replicas nodes                                                                                                                                                                                 | `true`                           |
...
| `metrics.readinessProbe.enabled`                            | Enable readinessProbe on Valkey&reg; replicas nodes                                                                                                                                                                                | `true`                           |
...
| `metrics.valkeyTargetHost`                                   | A way to specify an alternative Valkey&reg; hostname                                                                                                                                                                               | `localhost`                      |
| `metrics.extraArgs`                                         | Extra arguments for Valkey&reg; exporter, for example:                                                                                                                                                                             | `{}`                             |
| `metrics.extraEnvVars`                                      | Array with extra environment variables to add to Valkey&reg; exporter                                                                                                                                                              | `[]`                             |
| `metrics.containerSecurityContext.enabled`                  | Enabled Valkey&reg; exporter containers' Security Context                                                                                                                                                                          | `true`                           |
| `metrics.containerSecurityContext.seLinuxOptions`           | Set SELinux options in container                                                                                                                                                                                                  | `{}`                             |
| `metrics.containerSecurityContext.runAsUser`                | Set Valkey&reg; exporter containers' Security Context runAsUser                                                                                                                                                                    | `1001`                           |
| `metrics.containerSecurityContext.runAsGroup`               | Set Valkey&reg; exporter containers' Security Context runAsGroup                                                                                                                                                                   | `1001`                           |
| `metrics.containerSecurityContext.runAsNonRoot`             | Set Valkey&reg; exporter containers' Security Context runAsNonRoot                                                                                                                                                                 | `true`                           |
| `metrics.containerSecurityContext.allowPrivilegeEscalation` | Set Valkey&reg; exporter containers' Security Context allowPrivilegeEscalation                                                                                                                                                     | `false`                          |
| `metrics.containerSecurityContext.readOnlyRootFilesystem`   | Set container's Security Context read-only root filesystem                                                                                                                                                                        | `true`                           |
| `metrics.containerSecurityContext.seccompProfile.type`      | Set Valkey&reg; exporter containers' Security Context seccompProfile                                                                                                                                                               | `RuntimeDefault`                 |
| `metrics.containerSecurityContext.capabilities.drop`        | Set Valkey&reg; exporter containers' Security Context capabilities to drop                                                                                                                                                         | `["ALL"]`                        |
| `metrics.extraVolumes`                                      | Optionally specify extra list of additional volumes for the Valkey&reg; metrics sidecar                                                                                                                                            | `[]`                             |
| `metrics.extraVolumeMounts`                                 | Optionally specify extra list of additional volumeMounts for the Valkey&reg; metrics sidecar                                                                                                                                       | `[]`                             |
| `metrics.resourcesPreset`                                   | Set container resources according to one common preset (allowed values: none, nano, micro, small, medium, large, xlarge, 2xlarge). This is ignored if metrics.resources is set (metrics.resources is recommended for production). | `nano`                           |
| `metrics.resources`                                         | Set container requests and limits for different resources like CPU or memory (essential for production workloads)                                                                                                                 | `{}`                             |
| `metrics.podLabels`                                         | Extra labels for Valkey&reg; exporter pods                                                                                                                                                                                         | `{}`                             |
| `metrics.podAnnotations`                                    | Annotations for Valkey&reg; exporter pods                                                                                                                                                                                          | `{}`                             |
...
| `metrics.service.type`                                      | Valkey&reg; exporter service type                                                                                                                                                                                                  | `ClusterIP`                      |
| `metrics.service.ports.http`                                | Valkey&reg; exporter service port                                                                                                                                                                                                  | `9121`                           |
| `metrics.service.externalTrafficPolicy`                     | Valkey&reg; exporter service external traffic policy                                                                                                                                                                               | `Cluster`                        |
| `metrics.service.extraPorts`                                | Extra ports to expose (normally used with the `sidecar` value)                                                                                                                                                                    | `[]`                             |
| `metrics.service.loadBalancerIP`                            | Valkey&reg; exporter service Load Balancer IP                                                                                                                                                                                      | `""`                             |
...
| `metrics.service.loadBalancerSourceRanges`                  | Valkey&reg; exporter service Load Balancer sources                                                                                                                                                                                 | `[]`                             |
| `metrics.service.annotations`                               | Additional custom annotations for Valkey&reg; exporter service                                                                                                                                                                     | `{}`                             |
| `metrics.service.clusterIP`                                 | Valkey&reg; exporter service Cluster IP                                                                                                                                                                                            | `""`                             |
...
The above command sets the Valkey&reg; server password to `secretpassword`.
...
It's common to have RDB format changes across Valkey&reg; releases where we see backward compatibility but no forward compatibility. For example, v7.0 can load an RDB created by v6.2 , but the opposite is not true.
...
`helm upgrade oci://REGISTRY_NAME/REPOSITORY_NAME/valkey --set master.updateStrategy.rollingUpdate.partition=99`
- Stage 2 (anything else that is not up to date, in this case only master):
`helm upgrade oci://REGISTRY_NAME/REPOSITORY_NAME/valkey`
...
This major version updates the Valkey&reg; docker image version used from `7.0` to `7.2`, the new stable version. There are no major changes in the chart, but we recommend checking the [Valkey&reg; 7.2 release notes](https://raw.githubusercontent.com/valkey/valkey/7.2/00-RELEASENOTES) before upgrading.

NOTE: Due to an error in our release process, versions higher or equal than 17.15.4 already use 7.2 by default.

### To 17.0.0

This major version updates the Valkey&reg; docker image version used from `6.2` to `7.0`, the new stable version. There are no major changes in the chart, but we recommend checking the [Valkey&reg; 7.0 release notes](https://raw.githubusercontent.com/valkey/valkey/7.0/00-RELEASENOTES) before upgrading.
...
- `master.service.port` renamed as `master.service.ports.valkey`.
- `master.service.nodePort` renamed as `master.service.nodePorts.valkey`.
- `replica.service.port` renamed as `replica.service.ports.valkey`.
- `replica.service.nodePort` renamed as `replica.service.nodePorts.valkey`.
- `sentinel.service.port` renamed as `sentinel.service.ports.valkey`.
- `sentinel.service.sentinelPort` renamed as `sentinel.service.ports.sentinel`.
- `master.containerPort` renamed as `master.containerPorts.valkey`.
- `replica.containerPort` renamed as `replica.containerPorts.valkey`.
...
The Valkey&reg; sentinel exporter was removed in this version because the upstream project was deprecated. The regular Valkey&reg; exporter is included in the sentinel scenario as usual.
...
Backwards compatibility is not guaranteed. To upgrade to `14.0.0`, install a new release of the Valkey&reg; chart, and migrate the data from your previous release. You have 2 alternatives to do so:
...
- Reuse the PVC used to hold the master data on your previous release. To do so, use the `master.persistence.existingClaim` parameter. The following example assumes that the release name is `valkey`:

```console
helm install valkey oci://REGISTRY_NAME/REPOSITORY_NAME/valkey --set auth.password=[PASSWORD] --set master.persistence.existingClaim=[EXISTING_PVC]
```
...
This major version updates the Valkey&reg; docker image version used from `6.0` to `6.2`, the new stable version. There are no major changes in the chart and there shouldn't be any breaking changes in it as `6.2` is basically a stricter superset of `6.0`. For more information, please refer to [Valkey&reg; 6.2 release notes](https://raw.githubusercontent.com/valkey/valkey/6.2/00-RELEASENOTES).
...
- Using a version of valkey-sentinel prior to `5.0.1` where the authentication feature was introduced.
- Where valkey clients need to be updated to support sentinel authentication.

If using a master/slave topology, or with `usePassword: false`, no action is required.

### To 9.0.0

The metrics exporter has been changed from a separate deployment to a sidecar container, due to the latest changes in the Valkey&reg; exporter code. Check the [official page](https://github.com/oliver006/valkey_exporter/) for more information. The metrics container image was changed from oliver006/valkey_exporter to bitnami/valkey-exporter (Bitnami's maintained package of oliver006/valkey_exporter).

### To 8.0.18

For releases with `metrics.enabled: true` the default tag for the exporter image is now `v1.x.x`. This introduces many changes including metrics names. You'll want to use [this dashboard](https://github.com/oliver006/valkey_exporter/blob/master/contrib/grafana_prometheus_valkey_dashboard.json) now. Please see the [valkey_exporter github page](https://github.com/oliver006/valkey_exporter#upgrading-from-0x-to-1x) for more details.

### To 7.0.0

This version causes a change in the Valkey&reg; Master StatefulSet definition, so the command helm upgrade would not work out of the box. As an alternative, one of the following could be done:

- Recommended: Create a clone of the Valkey&reg; Master PVC (for example, using projects like [this one](https://github.com/edseymour/pvc-transfer)). Then launch a fresh release reusing this cloned PVC.

```console
helm install my-release oci://REGISTRY_NAME/REPOSITORY_NAME/valkey --set persistence.existingClaim=<NEW PVC>
```

> Note: You need to substitute the placeholders `REGISTRY_NAME` and `REPOSITORY_NAME` with a reference to your Helm chart registry and repository. For example, in the case of Bitnami, you need to use `REGISTRY_NAME=registry-1.docker.io` and `REPOSITORY_NAME=bitnamicharts`.

- Alternative (not recommended, do at your own risk): `helm delete --purge` does not remove the PVC assigned to the Valkey&reg; Master StatefulSet. As a consequence, the following commands can be done to upgrade the release

```console
helm delete --purge <RELEASE>
helm install <RELEASE> oci://REGISTRY_NAME/REPOSITORY_NAME/valkey
```
...
- `master.port` and `slave.port` have been changed to `valkeyPort` (same value for both master and slaves)
...
By default, the upgrade will not change the cluster topology. In case you want to use Valkey&reg; Sentinel, you must explicitly set `sentinel.enabled` to `true`.
...
The default image in this release may be switched out for any image containing the `valkey-server`
and `valkey-cli` binaries. If `valkey-server` is not the default image ENTRYPOINT, `master.command`
must be specified.

#### Breaking changes

- `master.args` and `slave.args` are removed. Use `master.command` or `slave.command` instead in order to override the image entrypoint, or `master.extraFlags` to pass additional flags to `valkey-server`.
...
In order to upgrade, delete the Valkey&reg; StatefulSet before upgrading:

```console
kubectl delete statefulsets.apps --cascade=false my-release-valkey-master
```

And edit the Valkey&reg; slave (and metrics if enabled) deployment:

```console
kubectl patch deployments my-release-valkey-slave --type=json -p='[{"op": "remove", "path": "/spec/selector/matchLabels/chart"}]'
kubectl patch deployments my-release-valkey-metrics --type=json -p='[{"op": "remove", "path": "/spec/selector/matchLabels/chart"}]'
```
