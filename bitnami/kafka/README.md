...
| `uncleanLeaderElectionEnable`              | Switch to enable replicas not in the ISR set to be elected as leader as a last resort, even though doing so may result in data loss                  | `false`                             |
...
| `replicaFetchMinBytes`                     | The minimum number of bytes expected for each fetch response                                                                                         | `_1`                                |
| `replicaFetchMaxBytes`                     | The number of bytes of messages to attempt to fetch for each partition                                                                               | `_1048576`                          |
| `minInsyncReplicas`                        | Minimum number of replicas that must acknowledge a write for being considered successful                                                             | `1`                                 |
