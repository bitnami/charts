...
- The standalone architecture (`architecture="standalone"`) installs a deployment with one InfluxDB server (it cannot be scaled).
- The high availability (`architecture="high-availability"`) install a statefulset with N InfluxDB servers and M InfluxDB Relay instances.
