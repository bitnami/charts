...
This chart supports creating a random admin token at first boot (not supported when using `memory` as object store) by setting the `auth.createAdminToken` parameter to `true`. Any further change in the credentials require manual intervention, pleaser refer instructions below:
...
| `auth.createAdminToken`                             | Whether to create a random admin token during container initialization (ignored if `objectStore` is set to `memory`)                                                                                              | `true`                     |
...
| `objectStore`                                       | InfluxDB(TM) Core object storage (using "file" implies deploying InfluxDB(TM) Core as a StatefulSet)                                                                                                              | `memory`                   |
| `nodeId`                                            | InfluxDB(TM) Core node id (ignored if `objectStore` is set to `file`)                                                                                                                                             | `0`                        |
| `databases`                                         | Comma separated list of databases to create (ignored if `objectStore` is set to `memory` or `auth.createAdminToken` is set to `false`)                                                                            | `""`                       |
...
| `replicaCount`                                      | Number of InfluxDB(TM) Core replicas (ignored if `objectStore` is set to `memory`)                                                                                                                                | `1`                        |
...
| `autoscaling.hpa.enabled`                           | Enable HPA for InfluxDB(TM) Core (ignored if `objectStore` is set to `memory`)                                                                                                                                    | `false`                    |
...
| `service.headless.annotations`          | Annotations for the headless service (only if `objectStore` is set to `file`)                                                    | `{}`                     |
| `service.headless.extraPorts`           | Optionally specify extra ports to expose for the headless service (only if `objectStore` is set to `file`)                       | `[]`                     |
