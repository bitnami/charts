...
This chart supports Ingress resources. If an Ingress controller, such as nginx-ingress or traefik, is installed in the cluster, that Ingress controller can be used to serve the application.

To enable Ingress integration, set `server.ingress.enabled` to `true` for the http ingress or `server.grpcIngress.enabled` to `true` for the gRPC ingress. The `*.ingress.hostname` property can be used to set the host name. The `*.ingress.tls` parameter can be used to add the TLS configuration for this host. It is also possible to have more than one host, with a separate TLS configuration for each host.
