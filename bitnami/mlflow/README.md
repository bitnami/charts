...
| `externalGCS.existingSecret`         | Name of an existing secret key containing the application credentials file (used only if useCredentialsInSecret is true) | `""`    |
| `externalGCS.existingSecretKey`      | Key in the existing secret containing the application credentials (must be set if existingSecret is set)                 | `""`    |
