...
This chart can be integrated with Prometheus by setting `*.metrics.enabled` (under the `dataPlane` and `controlPlane` sections) to true. This will expose the Apisix native Prometheus port in both the containers and services. The services will also have the necessary annotations to be automatically scraped by Prometheus.
