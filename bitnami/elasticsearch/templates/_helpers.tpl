...
{{/*
Returns true if at least 1 existing secret is missing
*/}}
{{- define "elasticsearch.security.tlsSecretsMissing" -}}
{{- $masterSecret :=.Values.security.tls.master.existingSecret -}}
{{- $dataSecret :=.Values.security.tls.data.existingSecret -}}
{{- $coordSecret :=.Values.security.tls.coordinating.existingSecret -}}
{{- $ingestSecret :=.Values.security.tls.ingest.existingSecret -}}
...
{{/* Validate values of Elasticsearch - Existing secret not provided for every role */}}
{{- define "elasticsearch.validateValues.security.missingTlsSecrets" -}}
{{- if and .Values.security.enabled (include "elasticsearch.security.tlsSecretsProvided" .) (include "elasticsearch.security.tlsSecretsMissing" .) -}}
elasticsearch: security.tls.*.existingSecret
    An existingSecret must be provided for each elasticsearch role in the cluster
    except for Ingest if disabled.
{{- end -}}
{{- end -}}
...
elasticsearch: security.enabled.tls
...
{{- $messages := append $messages (include "elasticsearch.validateValues.security.missingTlsSecrets" .) -}}
