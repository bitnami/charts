...
*/}}
{{- define "cilium.hubble.tls.client.secretName" -}}
{{- if or .Values.hubble.tls.autoGenerated.enabled (and (not (empty .Values.hubble.tls.client.cert)) (not (empty .Values.hubble.tls.client.key))) -}}
    {{- printf "%s-hubble-client-crt" (include "common.names.fullname" .) -}}
{{- else -}}
    {{- required "An existing secret name must be provided with TLS certs for Hubble client(s) if cert and key are not provided!" (tpl .Values.hubble.tls.client.existingSecret .) -}}
{{- end -}}
{{- end -}}
