...
var _ = Describe("MongoDB", Ordered, func() {
	...
	When("a database is created and Mongodb is scaled down to 0 replicas and back up", func() {
		It("should have access to the created database", func() {
			...
			ss, err := c.AppsV1().Deployments(*namespace).Get(ctx, *stsName, metav1.GetOptions{})
			Expect(err).NotTo(HaveOccurred())
			Expect(ss.Status.Replicas).NotTo(BeZero())
			origReplicas := *ss.Spec.Replicas

			Eventually(func() (*appsv1.Deployment, error) {
				return c.AppsV1().Deployments(*namespace).Get(ctx, *stsName, metav1.GetOptions{})
			...
			image, err := getContainerImage(ss, "mongodb")
			Expect(err).NotTo(HaveOccurred())
					...
					Containers: []v1.Container{
						{
							Name:    "mongodb",
							Image:   image,
							Command: []string{"mongosh", "--quiet", "--username", *username, "--password", *password, "--host", *stsName, "--port", port, "--eval", stmt},
							Env: []v1.EnvVar{
								{
