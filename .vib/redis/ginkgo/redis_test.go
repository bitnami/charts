...
var _ = Describe("Redis", Ordered, func() {
	...
	When("a key-value is created and Redis is scaled down to 0 replicas and back up", func() {
		It("should have access to the created key-value", func() {
			...
			port, err := getPort(svc, "tcp-redis")
			Expect(err).NotTo(HaveOccurred())

			image, err := getContainerImage(ss, "redis")
			Expect(err).NotTo(HaveOccurred())
					...
					Containers: []v1.Container{
						{
							Name:    "redis",
							Image:   image,
							Command: []string{"redis-cli", "-h", *stsName, "-p", port, stmt},
							Env: []v1.EnvVar{
								{
									Name:  "REDISCLI_AUTH",
									Value: *password,
