...
var _ = Describe("Zookeeper", Ordered, func() {
	...
	When("a key is created and Zookeeper is scaled down to 0 replicas and back up", func() {
		It("should have access to the created key", func() {
			...
			By("creating a job to drop the test key")
			deleteKeyJobName := fmt.Sprintf("%s-deletekey-%s",
				stsName, jobSuffix)
			err = createJob(ctx, c, deleteKeyJobName, port, image, "delete", keyName)
			Expect(err).NotTo(HaveOccurred())
