...
var _ = Describe("Kafka", Ordered, func() {
	...
	When("a topic is created and Kafka is scaled down to 0 replicas and back up", func() {
		It("should have access to the created topic", func() {
			...
			image, err := getContainerImage(ss, "kafka")
			Expect(err).NotTo(HaveOccurred())
			...
			Eventually(func() (*batchv1.Job, error) {
				return c.BatchV1().Jobs(*namespace).Get(ctx, createTPJobName, metav1.GetOptions{})
			...
			Eventually(func() (*batchv1.Job, error) {
				return c.BatchV1().Jobs(*namespace).Get(ctx, deleteTPJobName, metav1.GetOptions{})
			}, Timeout, PollingInterval).Should(WithTransform(getSucceededPods, Equal(int32(1))))
		})
	})
