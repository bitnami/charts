...
for item in *; do
  if [ -d "$item" ]; then
    ...
    if  grep -q 'containers@bitnami.com' 'Chart.yaml'; then
      ...
      sed -i 's/## Global Docker image registry/## Global Docker image parameters/g' values.yaml
      sed -i 's/## Please, note that this will override the image registry for all the images, including dependencies, configured to use the global value/## Please, note that this will override the image parameters, including dependencies, configured to use the global value\n## Current available global Docker image parameters: imageRegistry and imagepullSecrets/g' values.yaml
      sed -i 's/#   imageRegistry:/#   imageRegistry: myRegistryName\n#   imagePullSecrets:\n#     - myRegistryKeySecretName/g' values.yaml
      sed -i 's/myRegistrKeySecretName/myRegistryKeySecretName/g' values.yaml
      printf '\n{{/*\nReturn the proper Docker Image Registry Secret Names\n*/}}\n{{- define "%s.imagePullSecrets" -}}\n{{/*\nHelm 2.11 supports the assignment of a value to a variable defined in a different scope,\nbut Helm 2.9 and 2.10 does not support it, so we need to implement this if-else logic.\nAlso, we can not use a single if because lazy evaluation is not an option\n*/}}\n{{- if .Values.global }}\n{{- if .Values.global.imagePullSecrets }}\nimagePullSecrets:\n{{- range\n.Values.global.imagePullSecrets }}\n  - name: {{ . }}\n{{- end }}\n{{- else if or .Values.image.pullSecrets .Values.metrics.image.pullSecrets }}\nimagePullSecrets:\n{{- range .Values.image.pullSecrets }}\n  - name: {{ . }}\n{{- end }}\n{{- range .Values.metrics.image.pullSecrets }}\n  - name: {{ . }}\n{{- end }}\n{{- end -}}\n{{- else if or .Values.image.pullSecrets .Values.metrics.image.pullSecrets }}\nimagePullSecrets:\n{{- range .Values.image.pullSecrets }}\n  - name: {{ . }}\n{{- end }}\n{{- range .Values.metrics.image.pullSecrets }}\n  - name: {{ . }}\n{{- end }}\n{{- end -}}\n{{- end -}}' "$CHART" >> templates/_helpers.tpl
